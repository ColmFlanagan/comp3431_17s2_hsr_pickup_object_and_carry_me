...
class Arm_Controller:
    def __init__(self):
        ...
        self.Object_to_pick.camera_x = 0
        self.Object_to_pick.camera_y = 0
    ...
    def object_cam_x(self):
        return  self.Object_to_pick.camera_x
    def object_cam_y(self):
        return  self.Object_to_pick.camera_y    
    def object_name(self):
        return  self.Object_to_pick.name

    def get_angle(self):
        trans = ac.lookup_transform('/map', '/head_rgbd_sensor_rgb_frame')
        curr_x = trans[0]
        curr_y = trans[1]
        curr_z = trans[2]
        diffz = curr_z -ac.object_z()
        diffx = curr_x -ac.object_x()
        diffy = curr_y -ac.object_y()
        up_down = math.atan(diffz/diffx)
        left_right = math.atan(diffy/diffx)
...
def main(args):
    ...
    ac.reset_default_pos()
    ac.movehead()
    #ac.moveheadxy(-1.2,1.6)

    target_x = float(sys.argv[2])
    target_y = float(sys.argv[3])
    target_z = float(sys.argv[4])
    ac.objects_print()
    ac.get_object(OBJECT_TO_PICK)
    ac.object_print()
    if(ac.object_name() == OBJECT_TO_PICK):
        print("Objects x: ", ac.object_x())  
        print("Objects y: ", ac.object_y())  
        print("Objects z: ", ac.object_z())  


        headmovements = ac.get_angle(); 
        print("up_down: ", headmovements[0])  
        print("left_right: ", headmovements[1])       
        #ac.moveheadxy(headmovements[0],headmovements[1])
